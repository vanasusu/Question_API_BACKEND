...
router.get("/search", auth, async (req, res) => {
  try {
   ...
   var questions = await Question.find({})
    if (filters.level) {
     ques questions.filter(question=>question.level==filters.level)
    } 
    if(filters.topic) {
      questions.filter(question=>question.topic==filters.topic)
    }
    if(filters.subject) {
      questions.filter(question=>question.subject==filters.subject)
    }
    return res.status(200).json({ questions })
  } catch (error) {
    console.log(error);
